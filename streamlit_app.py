...
Runs the main app from app/main.py with cloud-compatible data caching.
"""
import sys
import os
    ...
    with col_charts:
        ...
        if isinstance(strategy, KeltnerMACDStrategy):
            with st.expander("Keltner Channel", expanded=True):
                kc = lib.keltner(period=strategy.kc_period, multiplier=strategy.kc_multiplier)
                st.plotly_chart(render_keltner_chart(df, kc), use_container_width=True)
            with st.expander("MACD", expanded=True):
                macd = lib.macd(fast=strategy.macd_fast, slow=strategy.macd_slow, signal=strategy.macd_signal)
